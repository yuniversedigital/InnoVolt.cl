...
// 💡 Eliminada la importación de InteractiveMapPlaceholder
...
const VisitaFormModal: React.FC<VisitaFormModalProps> = ({
  ...
  const [formData, setFormData] = useState({
    nombre: "",
    telefono: "",
    email: "", // Volvemos a un campo simple de dirección/referencia
    direccion: "", // Campos de Servicio
    // Campos de coordenadas eliminados: lat: 0, lng: 0,
    monto: 25000, // Precio fijo de la visita técnica
    problema: "",
    fechaTentativa: "",
  });

  const [isSubmitting, setIsSubmitting] = useState(false);
  const [error, setError] = useState<string | null>(null); // 💡 MANEJADOR DE UBICACIÓN ELIMINADO (handleLocationSelect)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError(null);
    setIsSubmitting(true); // 💡 VALIDACIÓN ACTUALIZADA para el campo 'direccion'
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/visitas`, {
        ...
        }, // Enviamos solo los campos básicos (sin lat/lng)
      ...
      if (!response.ok) {
        throw new Error(
          data.mensaje || "Error al registrar la solicitud de visita."
        );
      }
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div>
            ...
            <label
              ...
              className="text-gray-700 font-semibold flex items-center mb-1"
            >
                            <Send size={18} className="mr-2 text-blue-600" />{" "}
            ...
            <textarea
              id="problema"
              name="problema"
              placeholder="Ej: Falla eléctrica intermitente en el tablero principal."
          ...
          <div className="mt-6 pt-4 border-t border-gray-200">
            ...
            <p className="text-sm text-gray-500 mt-2">
                            *Este valor es por el diagnóstico y será descontado
              del costo               final si contratas la reparación o
              proyecto.            {" "}
            </p>
                     {" "}
          </div>
